...
from helper.config import load_config, disabled_features
...
if __name__ == "__main__":
    async def main():
        if not config.get("metafusion_run", True):
            logger.info("[MetaFusion] Processing is set to False. Exiting without changes.")
            return

        meta_banner(logger)
        logger.info(f"[MetaFusion] Started at {datetime.now().strftime('%Y-%m-%d %H:%M:%S')}")
        check_requirements(logger)
        disabled_features(config, logger)
        ...
        async with aiohttp.ClientSession() as session:
            try:
                ...
                cleanup_orphans = config["cleanup"].get("run_process", True)
                metadata_summaries = {}
                library_filesize = {}
