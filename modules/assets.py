...
def get_best_poster(
    ...
    if filtered:
        ...
        logging.debug(f"[Media Selection] Selected high-quality poster: {best}")
        return best
    ...
    if filtered:
        best = max(filtered, key=lambda x: (x["vote_average"], x["width"] * x["height"]))
        logging.debug(f"[Media Selection] Selected fallback poster: {best}")
        return best

    # Fallback: any poster
    best = max(images_to_consider, key=lambda x: (x["vote_average"], x["width"] * x["height"]))
    logging.debug(f"[Media Selection] Selected any available poster as final fallback: {best}")
    return best
...
def download_poster(image_path, save_path, item=None):
    ...
    try:
        ...
        logging.debug(f"[Media Download] Downloading {safe_title_year(item)} poster from URL: {url}")

        # Skip download if file already exists and not in dry run mode
        if save_path.exists() and not config.get("dry_run", False):
            logging.info(f"[Media Download] Poster {safe_title_year(item)} already exists. Skipping download.")
            return True
        ...
        if not response or response.status_code != 200:
            logging.warning(f"[Media Download] Failed to download image for {safe_title_year(item)}")
            return False

        try:
            save_poster(response.content, save_path, item)
            logging.debug(f"[Media Download] Downloaded poster for {safe_title_year(item)}")
            return True
        except Exception as e:
            logging.warning(f"[Media Download] Failed to save poster for {safe_title_year(item)}: {e}")
            return False

    except Exception as e:
        logging.warning(f"[Media Download] Failed to process poster download for {safe_title_year(item)}: {e}")
        return False
...
def should_upgrade(asset_path, new_image_data, new_image_path=None, cache_key=None, item=None, season_number=None):
    ...
    if not asset_path.exists():
        logging.info(f"[Media Download] No existing poster for {label}. Downloading new poster.")
        return not config.get("dry_run", False)
    ...
    if new_image_path and new_image_path.exists():
        try:
            ...
            logging.warning(f"[Media Checksum] Failed to read temp image for comparison: {e}")
            return not config.get("dry_run", False)
    else:
        logging.debug(f"[Media Checksum] No new_image_path provided for comparison. Skipping detailed check.")
        return False

    # Prefer new image if it's larger than existing
    if new_width > existing_width or new_height > existing_height:
        logging.info(
            f"[Media Recommendation] {label}: New {new_width}x{new_height}, "
            f"Existing {existing_width}x{existing_height}"
        )
        return not config.get("dry_run", False)

    logging.debug(f"[Media Upgrade] No upgrade needed for {safe_title_year(item)}. Existing image meets criteria.")
    return False
    ...
    try:
        ...
        if save_path.exists():
            ...
            if existing_checksum == new_checksum:
                logging.info(f"[Media Save] No changes detected for {safe_title_year(item)}. Skipping save.")
                return
            else:
                logging.info(f"[Media Save] Checksum difference detected for {safe_title_year(item)}. Proceeding to update.")
        ...
        logging.debug(f"[Media Save] Poster saved successfully for {safe_title_year(item)}")

    except Exception as e:
        logging.warning(f"[Media Save] Failed to save poster for {safe_title_year(item)}: {e}")